# Đường dẫn đến thư mục util
$UTIL_DIR = "src/main/java/com/moviebooking/util"
...
try {
    ...
    if (-not (Test-Path $UTIL_DIR)) {
        New-Item -Path $UTIL_DIR -ItemType Directory -Force
        Write-Host "Created directory: $UTIL_DIR"
    } else {
        Write-Host "Directory already exists: $UTIL_DIR"
    }
} catch {
...
# Tạo các file Util
foreach ($file_name in $UTIL_CONTENTS.Keys) {
    try {
        $file_path = Join-Path $UTIL_DIR $file_name
        Write-Host "Creating $file_path..."
        $UTIL_CONTENTS[$file_name] | Out-File -FilePath $file_path -Encoding UTF8
...
Write-Host "All Util files have been created in $UTIL_DIR"